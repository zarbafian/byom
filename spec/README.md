...
| `descriptors/` | machine-readable transition descriptors, one-to-one with every mutating operation in the bundle plus the named internal kernel transitions (§14.8) | society + participant/candidate + work-lifecycle + mandate/act-intent/charter machines (20 files); parity checked by the runner. C2 adds two Kovee-owned executor machines (`owner: "kovee (C2)"` — outside the BPP one-to-one rule): greenfield-enablement, endeavor-formation |
| `governed-work/` | C2 `byom_governed_work_v1` slice 1: the byom-normative binding/enablement/formation record schemas (§16.3/§16.6 field lists verbatim) plus the normative D10 greenfield enablement saga (`greenfield-saga.md`) | 16 closed record schemas; vectors under `vectors/governed-work/`; `proof/specs/GreenfieldEnablement.tla` |
