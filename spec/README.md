...
| `registry.json` | machine-readable operation registry: one row per `(operation, surface)` with binding (R-number), family, class (read/create/update), and the frozen request/result schema names (the RT-06 `-v2` successors included) — the freeze source for every bundle | LANDED (RT-12): the complete B0.1 bundle, 80 rows (the four G35 dual-surface ops with exactly two rows each); `../conformance/run.py` derives its bundle, meta-class, and MCP checks from these rows and fails on any extra/missing surface binding |
